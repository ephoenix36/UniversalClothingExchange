...
/**
 * Swaps Route Error Boundary
 * 
 * Catches errors specific to swap workflows.
 * Provides contextual recovery for swap operations.
 */
...
export default function SwapsError({ error, reset }: ErrorProps) {
  const errorInfo = formatErrorBoundaryError(error);

  useEffect(() => {
    logError(error, {
      route: '/swaps',
      component: 'SwapsErrorBoundary',
      action: 'swap workflow error',
    });
  }, [error]);
